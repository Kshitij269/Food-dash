            ...
            if ($user_id == '') {
                ...
                if (count($fetch_orders) > 0) {
                    for ($i = 0; $i < count($fetch_orders); $i++) {
                        ...
                        <div class="box">
                            <p>Placed on : <span>
                                    <?= $fetch_orders[$i]['placed_on']; ?>
                                </span></p>
                            <p>Name : <span>
                                    <?= $fetch_orders[$i]['name']; ?>
                                </span></p>
                            <p>Email : <span>
                                    <?= $fetch_orders[$i]['email']; ?>
                                </span></p>
                            <p>Number : <span>
                                    <?= $fetch_orders[$i]['number']; ?>
                                </span></p>
                            <p>Address : <span>
                                    <?= $fetch_orders[$i]['address']; ?>
                                </span></p>
                            <p>Payment method : <span>
                                    <?= $fetch_orders[$i]['method']; ?>
                                </span></p>
                            <p>Your orders : <span>
                                    <?= $fetch_orders[$i]['total_products']; ?>
                                </span></p>
                            <p>Total price : <span>$
                                    <?= $fetch_orders[$i]['total_price']; ?>/-
                                </span></p>
                            <p>Payment status : <span
                                    style="color:<?php if ($fetch_orders[$i]['payment_status'] == 'pending') {
                                        echo 'red';
                                    } else {
                                        echo 'green';
                                    }
                                    ; ?>">
                                    <?= $fetch_orders[$i]['payment_status']; ?>
                                </span> </p>
                        </div>
                        <?php
                    }
